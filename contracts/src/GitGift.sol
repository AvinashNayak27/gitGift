...
contract GitGift is SismoConnect {
  ...
  event Claimed(uint256 githubUserId, uint256 ethAmount);
  ...
  function donateETH(uint256 githubUserId) public payable {
    ...
    ethDonations[githubUserId].push(Donation({donor: payable(msg.sender), amount: msg.value}));
  ...
  function claimWithSismo(bytes memory response) public {
    ...
    require(ethDonations[processedUserId].length > 0, "No donations for this GitHub user");
    ...
    for (uint i = 0; i < ethDonations[processedUserId].length; i++) {
      ...
      emit Claimed(processedUserId, ethDonations[processedUserId][i].amount);
    }
  ...
  function findFirstNaturalNumberAfterSubstring(
